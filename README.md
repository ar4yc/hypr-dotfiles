> ## ⚠️ Warning! <br> * Project OUTDATED
<br> <br>
## My dotfiles for hyprland WM 

### Used:
* Terminal - Kitty
* Network Manager - NM-applet
* Bluetooth Manager - Overskride
* Volume Control - Pavucontrol
* File Manager - Dolphin
* App launcher - Wofi
* Top bar - Waybar
* Wallpapers - SwayBG, Waypaper
* Color Picker - Hyprpicker
* Screenshot - Flameshot

### Screenshots:
<img width="2560" height="1080" alt="Preview1" src="https://github.com/user-attachments/assets/202a372b-0a83-4090-8906-dc08e4727c7f" />
<br>
<img width="2560" height="1080" alt="Preview2" src="https://github.com/user-attachments/assets/8d1324e5-42d7-41d9-8944-3b7980304438" />

### Installation

* #### Clone dotfiles: #
```bash
git clone https://github.com/ar4yc/hypr-dotfiles.git && cp -r hypr-dotfiles/.config "$HOME/"
```
* #### Install Programs #
```bash
yay -S kitty dolphin wofi pavucontrol overskride-bin waybar cava nerd-fonts playerctl swaync gtk3 gtk4 qt6ct kvantum archlinux-xdg-menu btop wl-clipboard wtype cliphist flameshot-git hyprpicker swaybg waypaper hyprpolkitagent
```
